...
import com.duke.petnote.DEMO.DemoMainActivity;
import com.duke.petnote.R;
import com.duke.petnote.application.DemoApplication;
import com.duke.petnote.util.Constant;
import com.duke.petnote.util.HttpRequest;
import com.duke.library.base.BaseActivity;
import com.duke.library.interfaces.OnBottomDragListener;
import com.duke.library.interfaces.OnHttpResponseListener;
import com.duke.library.ui.WebViewActivity;
import com.duke.library.util.CommonUtil;
import com.duke.library.util.DownloadUtil;
...
public class AboutActivity extends BaseActivity implements OnClickListener, OnLongClickListener, OnBottomDragListener {
	...
	public void onClick(View v) {
		int id = v.getId();
		if (id == R.id.llAboutMainTabActivity) {
			startActivity(MainTabActivity.createIntent(context).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
			overridePendingTransition(com.duke.library.R.anim.bottom_push_in, com.duke.library.R.anim.hold);

			enterAnim = exitAnim = com.duke.library.R.anim.null_anim;
			finish();
		} else if (id == R.id.llAboutZBLibraryMainActivity) {
			startActivity(DemoMainActivity.createIntent(context));
			overridePendingTransition(com.duke.library.R.anim.bottom_push_in, com.duke.library.R.anim.hold);
		} else if (id == R.id.llAboutUpdate) {
			toActivity(WebViewActivity.createIntent(context, "更新日志", Constant.UPDATE_LOG_WEBSITE));
		} else if (id == R.id.llAboutShare) {
			CommonUtil.shareInfo(context, getString(com.duke.library.R.string.share_app) + "\n 点击链接直接查看ZBLibrary\n" + Constant.APP_DOWNLOAD_WEBSITE);
